...
Add `make dev-build` when packaging or assembled behavior is affected. Add the documented live CE MCP smoke flow only for MCP, Agent Mode, CLI, or running-app behavior. Launching or relaunching the production app requires explicit user approval.
