...
Focused validation and release validation remain explicit; use the validation matrix plus commands such as `make dev-release-preflight` / `make dev-release-artifact` when the changed boundary requires them. Obtain explicit user approval immediately before any force-push, history rewrite, branch deletion, fork deletion, credential rotation, or other GitHub-visible destructive mutation. Stopping, launching, or relaunching the visible RepoPrompt CE app also needs MP's approval, and the reason is the work inside it: an agent session or a Context Builder run that the stop would kill. When MP has said he is away or asleep and the app is idle, meaning no agent session is running and no Context Builder run is active, stopping production for a validation window and relaunching it before MP returns is within that approval; record the stop and relaunch times in the run's note.
...
`main` is the integration branch and the only long-lived branch. Cut a `bugfix/`, `port/`, or `chore/` branch from `main`, push it, and land it through a pull request on `mplibunao/repoprompt-ce-local` merged with a merge commit, so the review, the checks, and the diff stay auditable on GitHub. Nothing merges into `main` directly, and the `pr-ready` lane runs on the branch before the merge. The agent that owns a pull request opens it with one type label and the matching `area:` labels from the repository's label set, records the `pr-ready` result in its description, and drives the review bot's findings to a clean pass, running the commit and push preflights on every fix. A pull request that changes app code, anything under `Sources/` or `Packages/`, is validated in the debug app before it is ready for review: exercise the changed behavior through the debug CLI or the app itself, record the run and its result in the description, and until then keep it a draft that says validation is pending. Unit tests and `pr-ready` do not stand in for that run. Then MP reads and approves it; the agent then resolves any conflict with `main` on the branch through the same gates (the approval covers that commit, while any other later commit needs a fresh approval), reruns `pr-ready` on the final head with the upstream unset, and merges that exact head with a merge commit once its checks pass. A pull request is a draft while its agent is still fixing review-bot findings and is ready for review only when every thread is resolved, so the open, non-draft list is the review queue. When a change depends on one still under review, cut its branch from that branch, open the pull request against it, link the pair as a GitHub stack, and add the `stacked` label; GitHub then shows the layer, scopes the diff to it, and retargets it onto `main` when the lower one merges, rewriting its commits as it does; MP's approval of the lower layer covers that rewrite. [`CONTRIBUTING.md`](CONTRIBUTING.md) has the commands. Every promoted build is an annotated `local/v<version>-b<build>` tag on the promoted `main` commit; [`docs/releasing.md`](docs/releasing.md) owns the promotion procedure.

## Upstream is read-only

Three repositories share this code and each has a `main`; name them. *Upstream* is `repoprompt/repoprompt-ce`, the maintainers' repository, read only. *The fork* is `mplibunao/repoprompt-ce` on GitHub, frozen with its upstream pull requests open; nothing lands there. *The distribution* is `mplibunao/repoprompt-ce-local` on GitHub and this checkout on disk; every branch, pull request, issue, tag, and promoted build belongs to it, and an unqualified `main` means the distribution's `main`.
