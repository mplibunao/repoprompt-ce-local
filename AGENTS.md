...
Focused validation and release validation remain explicit; use the validation matrix plus commands such as `make dev-release-preflight` / `make dev-release-artifact` when the changed boundary requires them. Obtain explicit user approval immediately before any force-push, history rewrite, branch deletion, fork deletion, credential rotation, or other GitHub-visible destructive mutation. Stopping, launching, or relaunching the visible RepoPrompt CE app also needs MP's approval, and the reason is the work inside it: an agent session or a Context Builder run that the stop would kill. When MP has said he is away or asleep and no agent session is running in the app, stopping production for a validation window and relaunching it before MP returns is within that approval; record the stop and relaunch times in the run's note.
