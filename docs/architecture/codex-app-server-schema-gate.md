...
`CodexRuntimeAuthority.minimumExternalVersion`, the admission minimum for an explicitly selected
external executable. That external minimum remains 0.149.0 unless an observed consumed request or
...
exact release URLs and checksums, matching complete layouts, thin architectures, OpenAI signing
...
thread snapshot parser already classifies `interrupted` as terminal, so the compact contract expands
without adding a version adapter or weakening exhaustive enum validation.

Codex 0.153.4 exposes Astra through runtime model metadata. RepoPrompt continues to derive model
...
`NOTICE` are byte-identical to the 0.149.0 legal copies. Runtime selection remains bundled by default
