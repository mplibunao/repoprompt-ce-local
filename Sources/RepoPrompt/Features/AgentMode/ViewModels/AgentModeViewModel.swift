...
final class AgentModeViewModel: ObservableObject, CodexManagedSessionShutdownParticipant {
    ...
    /// Composer recovery: clear a saved pin whose value is no longer (or not yet) advertised.
    /// This is a local intent removal, not a provider selection — it never requires live
    /// parameter metadata and never invents a replacement value. Guards mirror the regular
    /// setter (inactive, non-MCP-controlled session whose selected provider and model still
    /// match the captured identity) so a stale menu action cannot clear a different target.
    /// Removing the identity advances its write revision, defeating an older MCP staging
    /// rollback that would otherwise restore the cleared pin.
    ///
    /// Live sessions need one extra step: OpenCode retains a previously applied effort across
    /// same-model re-selection (verified by the §5.1 capability gate), and an unpinned turn
    /// sends no effort mutation, so the session would keep running the cleared value while the
    /// chip shows the default. When the session still holds a controller and the held
    /// observation advertises a definition, apply that definition's current (default) value
    /// through the same controller path a run uses. Best-effort: the local removal stands even
    /// when the controller rejects the mutation.
    func clearSavedACPModelParameter(
        providerID: ACPProviderID,
        baseModelRaw: String,
        kind: ACPModelParameterKind
    ) async {
        guard selectedAgent.acpProviderID == providerID,
              let session = activeSession,
              !session.runState.isActive,
              !isMCPControlled(tabID: session.tabID),
              ACPModelParameterIdentity.canonicalBaseModelRaw(baseModelRaw, providerID: providerID)
              == ACPModelParameterIdentity.canonicalBaseModelRaw(selectedModelRaw, providerID: providerID)
        else { return }
        ...
        guard session.acpModelParameterSelections.contains(where: { $0.identity == identity }) else { return }

        // Capture the default application target before mutating intent: the live controller
        // plus the held observation's matching definition. The observation comes from a
        // disposable discovery session whose effort was never mutated, so its advertised
        // current value is the installation default.
        var defaultApplication: (controller: ACPAgentSessionController, selection: ACPModelParameterSelection)?
        if let controller = session.acpController,
           let observation = openCodeModelParameterObservation,
           observation.key == openCodeParameterDiscoveryKey(session: session, modelRaw: selectedModelRaw),
           case let .available(parameterSet) = observation.state,
           ACPModelParameterIdentity.canonicalBaseModelRaw(parameterSet.baseModelRaw, providerID: providerID)
           == ACPModelParameterIdentity.canonicalBaseModelRaw(selectedModelRaw, providerID: providerID),
           let definition = parameterSet.definition(kind: kind)
        {
            defaultApplication = (
                ...
                ACPModelParameterSelection(
                    ...
                    kind: definition.kind,
                    configID: definition.configID,
                    valueRaw: definition.currentValueRaw
                )
            )
        }

        session.acpModelParameterSelections = ACPModelParameterSelection.normalized(
            session.acpModelParameterSelections.filter { $0.identity != identity }
        )
        session.isDirty = true
        scheduleSave(for: session.tabID)
        syncComposerUIState()
        ...
        guard let defaultApplication else { return }
        _ = try? await defaultApplication.controller.applySessionModelParameterSelections(
            [defaultApplication.selection]
        )
    }
