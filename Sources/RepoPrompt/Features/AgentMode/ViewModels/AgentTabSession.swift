...
///
/// Extracted from `AgentModeViewModel` nesting so Agent Mode runtime contracts can
/// name the session type without the view-model namespace. This remains the single
/// session authority; `AgentModeViewModel.TabSession` is a source-compatibility
/// alias to exactly this class.
@MainActor
final class AgentTabSession: ObservableObject {
    ...
    var onSourceItemsChanged: ((AgentTabSession, AgentModeViewModel.SourceItemsMutation) -> Void)?
    ...
    var codexWatchdogState: AgentModeViewModel.CodexWatchdogState = .init()
    var codexNativeToolLiveness: AgentModeViewModel.CodexNativeToolLivenessState = .init()
    ...
    var pendingSourceItemsMutationSummary: AgentModeViewModel.PendingSourceItemsMutationSummary?
    var pendingDerivedTranscriptRefreshReason: AgentModeViewModel.DerivedTranscriptRefreshReason?
    var derivedTranscriptRefreshTask: Task<Void, Never>?
    var derivedTranscriptRefreshGeneration: UInt64 = 0
    var derivedTranscriptSyncState: AgentModeViewModel.DerivedTranscriptSyncState?
    ...
    var bashLiveExecutionByKey: [String: AgentModeViewModel.BashLiveExecutionState] = [:]
    var bashLiveExecutionKeyByTranscriptItemID: [UUID: String] = [:]

    /// Sequence tracking
    var nextSequenceIndex: Int = 0

    var bashLiveExecutionByTranscriptItemID: [UUID: AgentModeViewModel.BashLiveExecutionState] {
        var result: [UUID: AgentModeViewModel.BashLiveExecutionState] = [:]
    ...
    ///
    /// This method is purely synchronous and touches only AgentTabSession-owned
    ...
    func persistentBindingTransitionToken() -> AgentModeViewModel.PersistentBindingTransitionToken {
        AgentModeViewModel.PersistentBindingTransitionToken(
            tabID: tabID,
    ...
    /// `AgentTabSession` mutation helpers preserve via `reconcileEphemeralPayloadMap`.
    ...
    func bashLiveExecution(for transcriptItemID: UUID) -> AgentModeViewModel.BashLiveExecutionState? {
        guard let key = bashLiveExecutionKeyByTranscriptItemID[transcriptItemID] else { return nil }
        return bashLiveExecutionByKey[key]
    }

    func setBashLiveExecution(_ state: AgentModeViewModel.BashLiveExecutionState) {
    ...
    @discardableResult
    func removeBashLiveExecution(forKey key: String) -> AgentModeViewModel.BashLiveExecutionState? {
    ...
    @discardableResult
    func removeBashLiveExecution(forTranscriptItemID transcriptItemID: UUID) -> AgentModeViewModel.BashLiveExecutionState? {
        guard let key = bashLiveExecutionKeyByTranscriptItemID[transcriptItemID] else { return nil }
...
extension AgentModeViewModel {
    /// Source-compatibility alias for the extracted per-tab session type.
    typealias TabSession = AgentTabSession
}
