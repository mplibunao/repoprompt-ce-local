...
struct OpenCodeACPControllerModelDiscoveryClient: OpenCodeACPModelDiscoveryClient {
    ...
    func discoverModels(workspacePath: String?, modelRaw: String?) async throws -> OpenCodeACPModelDiscoveryResult? {
        ...
        // The capability opt-in suppresses the controller's global registry publication, so the
        // catalog must be read from the controller's own verified snapshot — reading the
        // registry here would return stale/empty data and silently drop the model list.
...
/// different key's result. The queue is FIFO. Each job also runs under a hard deadline
/// (`jobDeadlineNanos`, default 30 s): a wedged client is cancelled and awaited for cleanup,
/// its owners are settled `.failed`, and the single slot is released to the next queued job.
actor OpenCodeACPModelPollingService {
    ...
    private let jobDeadlineNanos: UInt64
    ...
    init(
        ...
        jobDeadlineNanos: UInt64 = 30_000_000_000
    ) {
        self.client = client
        self.intervalNanos = intervalNanos
        self.jobDeadlineNanos = jobDeadlineNanos
    }
    ...
    private func pumpRunner() {
        ...
        let deadlineNanos = jobDeadlineNanos
        activeTask = Task { [weak self] in
            // Run the disposable controller to completion, failure, or deadline, always through
            // the client, which shuts that controller down on success, failure, and
            // cancellation. The single-controller slot (`activeKey`) is released only from
            // `finishJob`, which runs strictly after the bounded run below has fully awaited
            // the client (including its shutdown `await`).
            let result: Result<OpenCodeACPModelDiscoveryResult?, Error>
            do {
                let value = try await Self.runBoundedJob(
                    client: client,
    ...
    /// Run one discovery job under a hard deadline. On expiry the work task is cancelled and
    /// then awaited — the client shuts its disposable controller down on cancellation — so the
    /// single active slot is never handed to the next job while a wedged controller still
    /// exists. The deadline error settles owners with `.failed(detail:)` through the normal
    /// failure path; it never impersonates caller cancellation.
    private static func runBoundedJob(
        client: any OpenCodeACPModelDiscoveryClient,
        workspacePath: String?,
        modelRaw: String?,
        deadlineNanos: UInt64
    ) async throws -> OpenCodeACPModelDiscoveryResult? {
        try await withThrowingTaskGroup(of: BoundedJobOutcome.self) { group in
            group.addTask {
                try await BoundedJobOutcome.completed(
                    client.discoverModels(workspacePath: workspacePath, modelRaw: modelRaw)
                )
            }
            group.addTask {
            ...
            case let .completed(value):
                group.cancelAll()
                _ = try? await group.next() // reap the timer child
                return value
            case .deadlineExpired:
                group.cancelAll()
                // Await the cancelled work child so the client's controller shutdown completes
                // before the slot is released (`finishJob` runs strictly after this returns).
                _ = try? await group.next()
                throw OpenCodeDiscoveryJobDeadlineError(deadlineNanos: deadlineNanos)
            }
        }
    }

    private enum BoundedJobOutcome {
        case completed(OpenCodeACPModelDiscoveryResult?)
        case deadlineExpired
    }

    /// A discovery job exceeded its hard deadline. Surfaced to observation owners as a
    /// terminal `.failed(detail:)` so chips and MCP callers recover instead of waiting
    /// forever on a wedged `opencode acp` process.
    private struct OpenCodeDiscoveryJobDeadlineError: LocalizedError {
        let deadlineNanos: UInt64

        var errorDescription: String? {
            let seconds = Double(deadlineNanos) / 1_000_000_000
            return String(format: "OpenCode model discovery exceeded its %.0f s job deadline.", seconds)
        }
    }
